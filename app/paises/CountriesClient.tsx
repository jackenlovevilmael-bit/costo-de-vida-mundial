        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full h-11 pl-11 pr-4 bg-white border border-slate-200 focus:border-emerald-300 rounded-2xl text-sm placeholder:text-slate-400 focus:outline-none transition-all"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-11 px-4 bg-white border border-slate-200 rounded-2xl text-sm focus:outline-none focus:border-emerald-300 min-w-[160px]"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 h-11 px-4 rounded-2xl text-sm border transition-all ${sortBy === "name" ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-medium" : "border-slate-200 hover:bg-slate-50"}`}
            >
              Nombre {sortBy === "name" && (sortOrder === "asc" ? "↑" : "↓")}
            </button>
            <button
              onClick={() => toggleSort("cost")}
              className={`flex items-center gap-1.5 h-11 px-4 rounded-2xl text-sm border transition-all ${sortBy === "cost" ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-medium" : "border-slate-200 hover:bg-slate-50"}`}
            >
              Costo {sortBy === "cost" && (sortOrder === "asc" ? "↑" : "↓")}
            </button>
            <button
              onClick={() => toggleSort("salary")}
              className={`flex items-center gap-1.5 h-11 px-4 rounded-2xl text-sm border transition-all ${sortBy === "salary" ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-medium" : "border-slate-200 hover:bg-slate-50"}`}
            >
              Salario {sortBy === "salary" && (sortOrder === "asc" ? "↑" : "↓")}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredAndSorted.map((country) => (
            <Link
              key={country.slug}
              href={`/${country.slug}`}
              className="group bg-white border border-slate-200 hover:border-emerald-300 rounded-3xl p-6 flex flex-col transition-all hover:shadow-md active:scale-[0.985]"
            >
              ...
              <div className="mt-auto pt-5 border-t grid grid-cols-3 gap-4 text-sm">
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-slate-400">SAL. MÍN.</div>
                  <div className="font-medium tabular-nums">€{country.minSalary}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-slate-400">SAL. PROM.</div>
                  <div className="font-medium tabular-nums">€{country.avgSalary}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-widest text-slate-400">ALQUILER</div>
                  <div className="font-medium tabular-nums">€{country.rent1BedCenter}</div>
                </div>
              </div>
            </Link>
          ))}
